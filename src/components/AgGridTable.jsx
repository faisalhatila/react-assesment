...
// const AgGridTable = ({ column = [], rows = [],isLoading,error }) => {
    ...
    <div className="w-full overflow-x-auto">
      <div className="min-w-[1024px] border border-gray-300 shadow-md rounded-md">
        ...
        <div
          ...
          style={{ scrollbarWidth: 'thin', scrollbarColor: '#d1d5db #f3f4f6' }} // Tailwind-like scrollbar styles for most browsers
        >
          {isLoading ? (
            ...
            rows.map((row, rowIndex) => (
              <div
                key={rowIndex}
                className="flex border-t cursor-pointer hover:bg-[#888] border-gray-200 p-3 items-center transition-all duration-300 ease-in-out group hover:bg-blue-50 hover:shadow-lg hover:scale-[1.01] relative"
              >
                {column.map((colItem, colIndex) => {
                  // Displaying an image if the column is 'image'
                  if (colItem === 'image') {
                ...
                {/* Optional row actions (hover effect) */}
                <div className="opacity-0 transform translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-300 flex gap-2 ml-2 absolute right-5">
                  <button className="px-3 py-1 text-xs font-medium text-blue-600 bg-blue-100 rounded hover:bg-blue-200 transition-colors duration-200">
                    Edit
                  </button>
                  <button className="px-3 py-1 text-xs font-medium text-red-600 bg-red-100 rounded hover:bg-red-200 transition-colors duration-200">
                    Delete
                  </button>
                </div>
              </div>
